...
from kp2d.utils.logging import SummaryWriter_, printcolor
...
def adjust_learning_rate(config, optimizer, epoch, decay=0.5, max_decays=4):
    ...
    exponent = min(epoch // (config.model.scheduler.lr_epoch_divide_frequency /
                   config.datasets.train.repeat), max_decays)
...
def main(file,pretrained_model):
    ...
    if(pretrained_model!="None"):
        print("Using pretrained Model",args.pretrained_model)
        checkpoint = torch.load(args.pretrained_model)
        model.load_state_dict(checkpoint['state_dict'])

    ######################################################################

    #print("Test Printing 1")
    train_dataset, train_loader = setup_datasets_and_dataloaders(
        config.datasets)
    #print("Test Printing 2")
    ...
    if rank() == 0:
        if not config.wandb.dry_run:
            print("Test 1")
            summary = SummaryWriter_(log_path,
                                     config,
            ...
            config.model.checkpoint_path = os.path.join(
                config.model.checkpoint_path, summary.run_name)
        else:
            print("Test 2")
            summary = SummaryWriter_(log_path,config)
            date_time = datetime.now().strftime("%m_%d_%Y__%H_%M_%S")
            date_time = model_submodule(
                model).__class__.__name__ + '_' + date_time
            config.model.checkpoint_path = os.path.join(
                config.model.checkpoint_path, date_time)
    ...
    printcolor('Training complete, models saved in {}'.format(
        config.model.checkpoint_path), "green")
...
def evaluation(config, completed_epoch, model, summary):
    ...
    if rank() == 0:
        ...
        for params in eval_params:
            hp_dataset = PatchesDataset(
                root_dir=config.datasets.val.path, use_color=use_color, output_shape=params['res'], type='a')
            ...
            printcolor(
                'Evaluating for {} -- top_k {}'.format(params['res'], params['top_k']))
            rep, loc, c1, c3, c5, mscore = evaluate_keypoint_net(data_loader,
                                                                 ...
                                                                 use_color=use_color)
            if summary:
                summary.add_scalar_('repeatability_'+str(params['res']), rep,completed_epoch)
                summary.add_scalar_('localization_' + str(params['res']), loc,completed_epoch)
                summary.add_scalar_(
                    'correctness_'+str(params['res'])+'_'+str(1), c1,completed_epoch)
                summary.add_scalar_(
                    'correctness_'+str(params['res'])+'_'+str(3), c3,completed_epoch)
                summary.add_scalar_(
                    'correctness_'+str(params['res'])+'_'+str(5), c5,completed_epoch)
                summary.add_scalar_('mscore' + str(params['res']), mscore,completed_epoch)
    ...
    if config.model.save_checkpoint and rank() == 0:
        current_model_path = os.path.join(
            config.model.checkpoint_path, 'model'+str(completed_epoch)+'.ckpt')
        current_model_keypoint_path = os.path.join(
            config.model.checkpoint_path, 'model_keypoint'+str(completed_epoch)+'.ckpt')
        
        printcolor('\nSaving model (epoch:{}) at {}'.format(
            completed_epoch, current_model_path), 'green')
        torch.save(
            {
                'state_dict': model_submodule(model_submodule(model).keypoint_net).state_dict(),
                'config': config
            }, current_model_keypoint_path)
        torch.save(
            {
...
def train(config, train_loader, model, optimizer, epoch, summary):
    ...
    pbar = tqdm(enumerate(train_loader, 0),
                unit=' images',
                unit_scale=config.datasets.train.batch_size * 1,
    ...
    for (i, data) in pbar:
        ...
        if 0 == 0:
            pbar.set_description('Train [ E {}, T {:d}, R {:.4f}, R_Avg {:.4f}, L {:.4f}, L_Avg {:.4f}]'.format(
                epoch, epoch * config.datasets.train.repeat, recall, running_recall /
                (i + 1), float(loss),
        ...
        if i % log_freq == 0:
            with torch.no_grad():
                if summary:
                    log_timestamp = (epoch)*n_train_batches + i
                    train_metrics = {
                    ...
                    for param_group in optimizer.param_groups:
                        train_metrics[param_group['name'] +
                                      '_learning_rate'] = param_group['lr']

                    for k, v in train_metrics.items():
                        summary.add_scalar_(k, v, log_timestamp)

                    model(data_cuda, debug=True)
                    for k, v in model_submodule(model).vis.items():
                        summary.add_image_(k, v, log_timestamp)
